...
class TestInicio(TestCase):
    ...
    def test_iniciar_sesion(self):
        response = self.client.get(
            '', {'username': 'AdminMFC', 'password': '1234'})
        self.assertEqual(200, response.status_code)
    ...
    def test_agregar_equipo(self):
        self.client.login(username='AdminMFC', password='1234')
        response = self.client.post('/nuevoEquipo/', {'cliente_equipo': 'no se', 'telefono_cliente': '1234567891', 'tipo_equipo': 'P', 'marca_equipo': 'hp',
                                    'modelo_equipo': '3ap2m', 'serial_number': '123ssa4a6a5s1', 'accesorios_equipo': 'ninguno', 'contraseña_equipo': '123', 'sistema_operativo': 'linux'})
        self.assertEqual(302, response.status_code)

    def test_agregar_equipo2(self):
        self.client.login(username='AdminMFC', password='1234')
        self.client.post('/nuevoEquipo/', {'cliente_equipo': "no se", 'telefono_cliente': 1234567891, 'tipo_equipo': 'P', 'marca_equipo': "Marca 1",
                         'modelo_equipo': "Modelo 1", 'serial_number': "123ssa4a6a5s1", 'accesorios_equipo': "ninguno", 'contraseña_equipo': "123", 'sistema_operativo': "linux"})
        self.assertEqual(1, Equipo.objects.count())

    def test_agregar_equipo_form_no_valido(self):
        self.client.login(username='AdminMFC', password='1234')
        response = self.client.post('/nuevoEquipo/', {'cliente_equipo': "no se", 'telefono_cliente': 1234567891, 'tipo_equipo': 'Portatil', 'marca_equipo': "Marca 1",
                                    'modelo_equipo': "Modelo 1", 'serial_number': "123ssa4a6a5s1", 'accesorios_equipo': "ninguno", 'contraseña_equipo': "123", 'sistema_operativo': "linux"})
        self.assertEqual(200, response.status_code)
    ...
    def test_registar_usuario(self):
        self.client.login(username='AdminMFC', password='1234')
        response = self.client.get(
            '/registrar/', {'username': 'anita24', 'password': '1234', 'password2': '1234'})
        self.assertEqual(200, response.status_code)
    ...
    def test_creacion_usuario_usuario_ya_existente(self):
        self.client.login(username='AdminMFC', password='1234')
        self.client.post(
            '/registrar/', {'username': 'user1', 'password': '1234', 'password2': '1234'})
        self.assertEqual(User.objects.count(), 2)

    def test_creacion_usuario_contraseña_diferente(self):
        self.client.login(username='AdminMFC', password='1234')
        response = self.client.post(
            '/registrar/', {'username': 'useraa', 'password': '123', 'password2': '1234'})
        self.assertEqual(200, response.status_code)
    ...
    def test_administrar_permisos(self):
        self.client.login(username='AdminMFC', password='1234')
        response = self.client.post(
            '/tipo_usuario/'+str(self.regular_user.id), {'grupos': '{self.technical_group}'})
        self.assertEqual(302, response.status_code)
    ...
    def test_editar_equipo(self):
        ...
        response = self.client.post('/equipos/editarEquipo/'+str(equipo.id), {'cliente_equipo': 'no se', 'telefono_cliente': '1234567891', 'tipo_equipo': 'P', 'marca_equipo': 'hp',
                                    'modelo_equipo': '3ap2m', 'serial_number': '123ssa4a6a5s1', 'accesorios_equipo': 'ninguno', 'contraseña_equipo': '123', 'sistema_operativo': 'linux'})
        self.assertEqual(302, response.status_code)
    ...
    def test_editar_equipo_form_no_valido(self):
        ...
        response = self.client.post('/equipos/editarEquipo/'+str(equipo.id), {'cliente_equipo': 'no se', 'telefono_cliente': '1234567891', 'tipo_equipo': 'Paaa', 'marca_equipo': 'hp',
                                    'modelo_equipo': '3ap2m', 'serial_number': '123ssa4a6a5s1', 'accesorios_equipo': 'ninguno', 'contraseña_equipo': '123', 'sistema_operativo': 'linux'})
        self.assertEqual(200, response.status_code)
    ...
    def test_eliminar_orden(self):
        ...
        response = self.client.get(
            '/servicios/eliminarOrden/'+str(orden_servicio.id_orden))
        self.assertEqual(302, response.status_code)
    ...
    def test_editar_orden(self):
        ...
        response = self.client.post('/servicios/editarOrden/'+str(orden_servicio.id_orden), {'fecha_entrega': '09/12/2023', 'cotizacion': '123', 'costo_final': '123', 'tipo_servicio': 'limpieza', 'falla_equipo': 'ninguna',
                                    'indicaciones_adicionales': 'sin indicaciones', 'servicio_realizado': 'limpieza profunda', 'notas_finales': 'lista para entrega', 'encargado': 'EspaSandia24', 'observaciones_notas': 'ninguna', 'partes': 'ninguna'})
        self.assertEqual(302, response.status_code)
    ...
    def test_editar_orden_form_no_valido(self):
        ...
        response = self.client.post('/servicios/editarOrden/'+str(orden_servicio.id_orden), {'fecha_entrega': '09/12/', 'cotizacion': '123', 'costo_final': '123', 'tipo_servicio': 'limpieza', 'falla_equipo': 'ninguna',
                                    'indicaciones_adicionales': 'sin indicaciones', 'servicio_realizado': 'limpieza profunda', 'notas_finales': 'lista para entrega', 'encargado': 'EspaSandia24', 'observaciones_notas': 'ninguna', 'partes': 'ninguna'})
        self.assertEqual(200, response.status_code)
    ...
    def test_editar_orden_status_200(self):
        ...
        response = self.client.get(
            '/servicios/editarOrden/'+str(orden_servicio.id_orden))
        self.assertEqual(200, response.status_code)
